...
class DataBaseManager:
    ...
    def create_schema(self):
        ...
        cursor.execute('DROP TABLE IF EXISTS "consumerOffsets"')
        cursor.execute('DROP TABLE IF EXISTS "producerStreamRelations"')
        ...
        cursor.execute('''CREATE UNIQUE INDEX events_streamname_eventname_sequence_eventid_uindex
                        ON events ("streamName", "eventName", sequence, "eventId");''')
        cursor.execute('''CREATE INDEX events_payload ON events((event->>'payload'));''')
        cursor.execute('''CREATE TABLE IF NOT EXISTS "consumerOffsets"
                (
        ...
        cursor.execute('''CREATE UNIQUE INDEX consumeroffsets_consumerid_streamname_eventname_uindex
                        ON "consumerOffsets" ("consumerId", "streamName", "eventName");''')
        cursor.execute('''CREATE TABLE IF NOT EXISTS "producerStreamRelations"
                (
                  "producerId" CHAR(36)     NOT NULL,
                  "streamName" VARCHAR(255) NOT NULL
                );''')
        cursor.execute('''CREATE UNIQUE INDEX producerstreamrelations_streamname_uindex
                        ON "producerStreamRelations" ("streamName");''')
        cursor.execute('''CREATE OR REPLACE FUNCTION ranking(numeric, numeric[], int)
                    RETURNS integer AS $$
                DECLARE
                ...
                END;
                $$ LANGUAGE plpgsql;''')
        cursor.execute('''CREATE OR REPLACE FUNCTION search_result(text)
                    RETURNS TABLE(
                        label    text,
                        category text,
                        url      text
    ...
    def truncate_tables(self):
        ...
        cursor.execute('TRUNCATE table "consumerOffsets"')
        cursor.execute('TRUNCATE table "producerStreamRelations"')
